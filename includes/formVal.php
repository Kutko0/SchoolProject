...
if(isset($_POST['loginBtn']) ){
	...
	|| (requiredInput($_POST['ucitelVyber']) !== TRUE)){
		...
		if(haveInfo($hash_id)){
			...
			redirect('user/index.php?ui=prih&ni=1');
...
}else if(isset($_POST['commentBtnForTeacher'])){
	$sprava = $_POST['comment'];
	$student_hash = $_POST['commentBtnForTeacher'];

	if(isTeacher($_SESSION['hash_id'])){
		if(requiredInput($sprava)){
			if(addComment($student_hash, $sprava, TRUE)){
				redirect('user/index.php?ti=mz&s=1');
			}else{
				redirect('user/index.php?ti=mz&f=2');
			}
		}else{
			redirect('user/index.php?ti=mz&ff=1');
		}
	}else{
		redirect('user/index.php?ti=mz&f=1');
	}
